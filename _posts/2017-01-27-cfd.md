...
- The effects of fluid dynamics are often unintuitive (at least initially) and having immediate feedback through visualizations and statictics from an advanced CFD solver running in parallel will provide a method for the designer to study the subtle effects of form vs air flow on a building. 
...
- While it is common practice for a large engineering company (ie. aerospace or automotive) to comission a full scale physical prototype to verrify performance, in the architecture industry, it is extremely cost prohibitive to build a model for wind tunnel testing on most buildings except for large scale high budget projects. Computer simulations are cheaper, faster, and more acessible. They also provide the architect information that are impractical for physical prototypes, such as detailed room by room interior air flow or pressure + temperature gradients.
...
***Morphosis - SF Federal Building:*** One of the goals of this building was to utilize 100% natural ventilation throughout the tower. Designers even considered micro climate changes from possible future neighboring buildings as they alter city and street scale wind patterns. 
...
These metrics fit into three categories - flow control at a specific location, temperature gradient between separate components, and turbelance created by a given geometry.  
...
```
Post processing in The Living's normal optimization workflows typically happen in a combination of D3 for data visualization and MS Excel. The results that are output from a standard SimCFD analysis are pretty broad, but the main metrics to design for are volume flow, pressure, temperature, and velocity. 
